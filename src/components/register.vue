          ...
          if (flag == 'Exist') {
            ...
            axios({
              ...
              if (response.status == 200) {
                self.errinfo = '注册成功，欢饮您，' + name;
                self.classFade = ''
                //    setCookie.getInfo(this.userName,this.pwd);
                //    setCookie.userLogin();
                setTimeout("window.location.href = './#/login'", 2000)
              } else {
                self.errinfo = '注册失败，位置错误';
                self.classFade = ''
              }
            }).catch(function(error) {
              console.log(error);
              self.classFade = '';
              self.errinfo = '服務器繁忙，請刷新頁面或者稍後重試!(Error code:1 504)'
            });
          }
        }).catch(function(error) {
          console.log(error);
          self.classFade = '';
          self.errinfo = '服務器繁忙，請刷新頁面或者稍後重試!(Error code:2 504)'
        });
      }
    },
    closemodel: function() {
