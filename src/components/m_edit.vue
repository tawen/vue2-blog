...
<div class='main'>
  ...
  <span class='text_center btn_style'>
		<button class='submit_btn' @click='written'>留言</button>
		<button class='submit_btn ml2' @click='show_info'>查看留言</button>
		</span>
...
export default {
  ...
  methods: {
    written: function() {
      ...
      if (message_info != '' && this.author != '') {
        axios({
          ...
          console.log(error);
          self.classFade = '';
          self.errinfo = '服务器繁忙，请刷新页面重试!(Error code: 504)'
        });
      } else if (this.author == '') {
        this.classFade = '';
    ...
    closemodel: function() {
      this.classFade = 'hide';
      if (this.errinfo == '留言成功！') {
        this.$router.push("/message_board/message_show");
      }
    },
    show_info() {
      this.author = unescape(setCookie.setInfo().name);
